...
async fn create_table_insert_select() {
    ...
    client.execute("INSERT INTO users (id, name) VALUES (1, 'Alice')", &[]).await.unwrap();
    client.execute("INSERT INTO users (id, name) VALUES (2, 'Bob')", &[]).await.unwrap();

    let rows = client.query("SELECT id, name FROM users ORDER BY id", &[]).await.unwrap();
...
async fn update_and_delete() {
    ...
    client.execute("CREATE TABLE items (id INTEGER PRIMARY KEY, qty INTEGER)", &[]).await.unwrap();
    client.execute("INSERT INTO items VALUES (1, 10)", &[]).await.unwrap();
    client.execute("INSERT INTO items VALUES (2, 20)", &[]).await.unwrap();

    client.execute("UPDATE items SET qty = 15 WHERE id = 1", &[]).await.unwrap();

    let rows = client.query("SELECT id, qty FROM items ORDER BY id", &[]).await.unwrap();
    assert_eq!(rows.len(), 2);
    let qty: i64 = rows[0].get(1);
    assert_eq!(qty, 15);

    client.execute("DELETE FROM items WHERE id = 2", &[]).await.unwrap();

    let rows = client.query("SELECT id, qty FROM items", &[]).await.unwrap();
    assert_eq!(rows.len(), 1);
    let id: i64 = rows[0].get(0);
    assert_eq!(id, 1);
}
...
async fn multiple_connections() {
    ...
    client1.execute("CREATE TABLE shared (id INTEGER PRIMARY KEY, val TEXT)", &[]).await.unwrap();
    client1.execute("INSERT INTO shared VALUES (1, 'from_conn1')", &[]).await.unwrap();
    drop(client1);

    // Second connection should see the data.
    let client2 = connect(port).await;
    let rows = client2.query("SELECT id, val FROM shared", &[]).await.unwrap();
    assert_eq!(rows.len(), 1);
...
async fn empty_table_query() {
    ...
    client.execute("CREATE TABLE empty_t (id INTEGER PRIMARY KEY, val TEXT)", &[]).await.unwrap();
...
async fn drop_table() {
    ...
    client.execute("CREATE TABLE temp_t (id INTEGER PRIMARY KEY)", &[]).await.unwrap();
...
async fn pg_serial_to_integer() {
    ...
    client.execute("CREATE TABLE auto_t (id SERIAL PRIMARY KEY, name TEXT)", &[]).await.unwrap();

    client.execute("INSERT INTO auto_t (name) VALUES ('first')", &[]).await.unwrap();
    client.execute("INSERT INTO auto_t (name) VALUES ('second')", &[]).await.unwrap();

    let rows = client.query("SELECT id, name FROM auto_t ORDER BY id", &[]).await.unwrap();
    assert_eq!(rows.len(), 2);
    let name: &str = rows[0].get(1);
...
async fn pg_varchar_to_text() {
    ...
    client
        .execute("CREATE TABLE typed_t (name VARCHAR(255), bio TEXT, data BYTEA)", &[])
        .await
        .unwrap();

    client
        .execute("INSERT INTO typed_t (name, bio, data) VALUES ('test', 'a bio', 'raw')", &[])
        .await
        .unwrap();

    let rows = client.query("SELECT name, bio FROM typed_t", &[]).await.unwrap();
    assert_eq!(rows.len(), 1);
...
async fn pg_boolean_column() {
    ...
    client
        .execute("CREATE TABLE flags (id INTEGER PRIMARY KEY, active BOOLEAN)", &[])
        .await
        .unwrap();

    client.execute("INSERT INTO flags VALUES (1, TRUE)", &[]).await.unwrap();
    client.execute("INSERT INTO flags VALUES (2, FALSE)", &[]).await.unwrap();

    let rows = client.query("SELECT id, active FROM flags ORDER BY id", &[]).await.unwrap();
    assert_eq!(rows.len(), 2);
...
async fn large_result_set() {
    ...
    client.execute("CREATE TABLE big_t (id INTEGER PRIMARY KEY, val TEXT)", &[]).await.unwrap();

    // Insert 100 rows.
    for i in 0..100 {
        client.execute(&format!("INSERT INTO big_t VALUES ({i}, 'row_{i}')"), &[]).await.unwrap();
    }

    let rows = client.query("SELECT id, val FROM big_t ORDER BY id", &[]).await.unwrap();
...
async fn null_handling() {
    ...
    client.execute("CREATE TABLE nullable (id INTEGER PRIMARY KEY, val TEXT)", &[]).await.unwrap();

    client.execute("INSERT INTO nullable VALUES (1, NULL)", &[]).await.unwrap();
    client.execute("INSERT INTO nullable VALUES (2, 'present')", &[]).await.unwrap();

    let rows = client.query("SELECT id, val FROM nullable ORDER BY id", &[]).await.unwrap();
    assert_eq!(rows.len(), 2);
...
async fn transaction_commit() {
    ...
    client.batch_execute("CREATE TABLE txn_t (id INTEGER PRIMARY KEY, val TEXT)").await.unwrap();

    client.batch_execute("BEGIN").await.unwrap();
    client.batch_execute("INSERT INTO txn_t VALUES (1, 'inside_txn')").await.unwrap();
    client.batch_execute("COMMIT").await.unwrap();

    // Data should be visible after commit.
    let rows = client.query("SELECT id, val FROM txn_t", &[]).await.unwrap();
    assert_eq!(rows.len(), 1);
    let val: &str = rows[0].get(1);
...
async fn transaction_rollback() {
    ...
    client.batch_execute("CREATE TABLE txn_rb (id INTEGER PRIMARY KEY, val TEXT)").await.unwrap();

    client.batch_execute("INSERT INTO txn_rb VALUES (1, 'before')").await.unwrap();

    client.batch_execute("BEGIN").await.unwrap();
    client.batch_execute("INSERT INTO txn_rb VALUES (2, 'rolled_back')").await.unwrap();
    client.batch_execute("ROLLBACK").await.unwrap();

    // Only the row inserted before the transaction should exist.
    let rows = client.query("SELECT id, val FROM txn_rb ORDER BY id", &[]).await.unwrap();
    assert_eq!(rows.len(), 1);
    let val: &str = rows[0].get(1);
...
async fn transaction_atomicity() {
    ...
    client.batch_execute("INSERT INTO txn_atom VALUES (1, 100)").await.unwrap();

    // Begin a transaction, update, then rollback — value should remain 100.
    client.batch_execute("BEGIN").await.unwrap();
    client.batch_execute("UPDATE txn_atom SET val = 200 WHERE id = 1").await.unwrap();
    client.batch_execute("ROLLBACK").await.unwrap();

    let rows = client.query("SELECT id, val FROM txn_atom", &[]).await.unwrap();
    assert_eq!(rows.len(), 1);
...
async fn float_values() {
    ...
    client.execute("CREATE TABLE floats (id INTEGER PRIMARY KEY, val REAL)", &[]).await.unwrap();

    client.execute("INSERT INTO floats VALUES (1, 3.14)", &[]).await.unwrap();

    let rows = client.query("SELECT val FROM floats WHERE id = 1", &[]).await.unwrap();
